...
def update_trending_repo_names_collection(trending_repo_names_list):
    ...
    if 'trending_repos' in trending_repo_names_list:
        trending_repo_names = db.get_collection('trending_repos').distinct("name")
...
def populate_label_data(trending_labels, np_list_1, np_list_2, np_list_3):
    ...
    for i in range(get_list_size(np_list_1)):
        ...
        tuple = {'company': np_list_1[i:i + 1, 0][0], 'label': np_list_1[i:i + 1, 1][0]}
        db['trending_labels'].insert_one(tuple)

    for i in range(get_list_size(np_list_2)):
        tuple = {'company': np_list_2[i:i + 1, 0][0], 'label': np_list_2[i:i + 1, 1][0]}
        db['trending_labels'].insert_one(tuple)

    for i in range(get_list_size(np_list_3)):
        tuple = {'company': np_list_3[i:i + 1, 0][0], 'label': np_list_3[i:i + 1, 1][0]}
        db['trending_labels'].insert_one(tuple)
...
# extract labels from issues in 'trending_repos' collection
repo_labels_ex = RepoLabel()
repo_labels = repo_labels_ex.get_repo_labels(collections)

# extract labels from pull requests in 'trending_repos' collection
